...
def generate_monaco_completion(items):
    ...
    js_text = f"""
    export const COMPLETION_DATA = [{','.join(_item(**x) for x in items)}];
    """
    return js_text
...
def _monaco_completion(g):
    ...
    Path("docs/static/d3fendData.ts").write_text(
        f"""
...
def _d3fend_short(g):
    g.serialize("docs/static/d3fend.ttl")
    subset = tuple(
        g.query(
...
if __name__ == "__main__":
    ...
    for f in FILES:
        if not Path(f['src']).exists(): continue
